<script>
window.print();
</script>

<?php
  $tanggal=date_default_timezone_get();
 ?>

<center>
  <p>{!! Html::image('kop copy.png','a picture', array('style'=>'width:902px; height:159px')) !!}
  </p></center>
<h3><b><center>
  <p>BERITA ACARA UJIAN PROPOSAL </p>
</center></h3>
<center><table width="712" border="0">
  <tr>
    <td colspan="3"><div align="justify">Pada Hari tanggal {!! date('d-M-Y',strtotime($tanggal)) !!}, telah dilaksanakan ujian proposal mahasiswa :</div></td>
    </tr>
  <tr>
    <td width="210"><div align="left">Nama Mahasiswa</div></td>
    <td width="19"><div align="center">:</div></td>
    <td width="461">{!! $ujianta1->mahasiswa->nama !!}</td>
  </tr>
  <tr>
    <td><div align="left">NIM</div></td>
    <td><div align="center">:</div></td>
    <td>{!! $ujianta1->mahasiswa->nim !!}</td>
  </tr>
  <tr>
    <td><div align="left">Program Studi</div></td>
    <td><div align="center">:</div></td>
    <td>{!! $ujianta1->mahasiswa->prodi->nama !!}</td>
  </tr>
  <tr>
    <td><div align="left">Hari/Tanggal</div></td>
    <td><div align="center">:</div></td>
    <td>{!! date('d-M-Y',strtotime($tanggal)) !!}</td>
  </tr>
  <tr>
    <td><div align="left">Judul Tugas Akhir</div></td>
    <td><div align="center">:</div></td>
    <td>{!! $pengajuan->judul !!}</td>
  </tr>
  <tr>
    <td><div align="left">Pembimbing I</div></td>
    <td><div align="center">:</div></td>
    <td>{!! $pengajuan->ppta->data_pembimbing_1->nama!!}</td>
  </tr>
  <tr>
    <td><div align="left">Pembimbing II</div></td>
    <td><div align="center">:</div></td>
    <td>{!! @$pengajuan->ppta->data_pembimbing_2->nama!!}</td>
  </tr>
  <tr>
    <td><div align="left">Dengan Penguji</div></td>
    <td><div align="center">:</div></td>
    <td></td>
  </tr>
</table></center>
<center><table width="710" border="1">
  <tr>
    <td width="34"><div align="center">No.</div></td>
    <td width="222"><div align="center">Nama Penguji</div></td>
    <td width="218"><div align="center">Status Penguji</div></td>
    <td width="208"><div align="center">Tanda Tangan</div></td>
  </tr>
  <tr>
    <td><div align="center">1.</div></td>
    <td>{!! $pengajuan->ppta->data_pembimbing_1->nama!!}</td>
    <td><div align="center">Ketua</div></td>
    <td>&nbsp;</td>
  </tr>
  <tr>
    <td><div align="center">2.</div></td>
    <td>{!! @$pengajuan->ppta->data_pembimbing_2->nama!!}</td>
    <td><div align="center">Sekretaris</div></td>
    <td>&nbsp;</td>
  </tr>
  <tr>
    <td><div align="center">3.</div></td>
    <td>{!! $penguji->data_penguji_1->nama !!}</td>
    <td><div align="center">Anggota</div></td>
    <td>&nbsp;</td>
  </tr>
  <tr>
    <td><div align="center">4.</div></td>
    <td>{!! $penguji->data_penguji_2->nama !!}</td>
    <td><div align="center">Anggota</div></td>
    <td>&nbsp;</td>
  </tr>
</table>
  <p>&nbsp;</p>
  <table width="703" border="0">
    <tr>
      <td colspan="2"><div align="right">Palembang, {!! date('d-M-Y',strtotime($tanggal)) !!}</div></td>
    </tr>
    <tr>
      <td width="359">Mengatahui</td>
      <td width="334">&nbsp;</td>
    </tr>
    <tr>
      <td>Ketua Jurusan,</td>
      <td>Ketua Tim Penguji,</td>
    </tr>
    <tr>
      <td>&nbsp;</td>
      <td>&nbsp;</td>
    </tr>
    <tr>
      <td>&nbsp;</td>
      <td>&nbsp;</td>
    </tr>
    <tr>
      <td>&nbsp;</td>
      <td>&nbsp;</td>
    </tr>
    <tr>
      <td>&nbsp;</td>
      <td>&nbsp;</td>
    </tr>
    <tr>
      <td>Fathoni, MMSI</td>
      <td>{!! $pengajuan->ppta->data_pembimbing_1->nama!!}</td>
    </tr>
    <tr>
      <td>NIP. 197210182008121001</td>
      <td>{!! $pengajuan->ppta->data_pembimbing_1->nip!!}</td>
    </tr>
  </table>
  <p>&nbsp;</p>
</center>
<p>&nbsp;</p>
